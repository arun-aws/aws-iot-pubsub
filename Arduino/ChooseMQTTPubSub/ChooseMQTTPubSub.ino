...
const char MQTT_SUB_TOPIC[] = "remoteCar";
const char MQTT_PUB_TOPIC[] = "remoteCar";
...
#else
  WiFi.hostname(THINGNAME); // Arun AWS removed
#endif
  WiFi.mode(WIFI_STA);
...
#else
  net.setTrustAnchors(&cert); // Arun AWS removed
  net.setClientRSACert(&client_crt, &key); // Arun AWS removed
#endif
...
#else
  client.begin(MQTT_HOST, MQTT_PORT, net); // Arun AWS removed
  client.onMessage(messageReceived); // Arun AWS removed
#endif
  connectToMqtt(); // Arun AWS removed
}

void loop()
{
  now = time(nullptr);
  // Arun AWS removed starts here
  if (!client.connected())
  {
    checkWiFiThenMQTT();
  ...
  else
  {
    ...
    {
      lastMillis = millis();
      // sendData(); // Arun Publish removed
    }
  }
  // Arun AWS removed ends here
  if (millis() - lastMillis > 5000) {
    lastMillis = millis();
    Serial.println("Message from ESP");
    recievedtext = Serial.readString();
